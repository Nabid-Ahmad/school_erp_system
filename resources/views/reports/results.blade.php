...
<body>
    <div class="no-print" style="margin-bottom: 20px;">
        <button onclick="window.print()" style="background: #4f46e5; color: #fff; border: none; padding: 10px 20px; border-radius: 8px; font-weight: bold; cursor: pointer;">
    ...
    <table class="header-table">
        <tr>
            <td>
                ...
                <div class="report-sub">ACADEMIC EXAM RESULTS REPORT</div>
                <div class="report-sub">Class: {{ $selectedClass ? 'Class '.$selectedClass->name : 'All Classes' }}</div>
            </td>
    ...
    <table class="report-table">
        ...
        <tbody>
            @forelse($results as $res)
                <tr>
                    <td><strong>{{ $res->student->roll ?? 'N/A' }}</strong></td>
                    <td><strong>{{ $res->student->name ?? 'Student' }}</strong></td>
                    <td>Class {{ $res->student->schoolClass->name ?? 'N/A' }}</td>
                    <td>{{ $res->subject->name ?? 'Subject' }}</td>
                    <td><strong>{{ $res->marks }} / 100</strong></td>
                    <td><span style="background: #e0e7ff; color: #3730a3; padding: 2px 6px; border-radius: 4px; font-weight: 900;">{{ $res->grade ?? 'N/A' }}</span></td>
                </tr>
            @empty
                <tr><td colspan="6" style="text-align: center; color: #94a3b8;">No exam result records found.</td></tr>
            @endforelse
        </tbody>
    </table>

    <div class="sign-area">
        <div class="sign-col"><div class="sign-line"></div><div class="sign-label">Exam Controller</div></div>
