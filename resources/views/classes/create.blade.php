            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('classes.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            <label for="name" class="block text-sm font-medium text-gray-700">Class Name (e.g. 6, 7, 8, Nine)</label>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="mb-4">
                                ...
                                <input type="number" step="0.01" name="monthly_fee" id="monthly_fee" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm" value="{{ old('monthly_fee', 0) }}" required>
                                @error('monthly_fee')
                                    <p class="text-danger text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <input type="number" step="0.01" name="admission_fee" id="admission_fee" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-primary focus:ring-primary sm:text-sm" value="{{ old('admission_fee', 0) }}" required>
                                @error('admission_fee')
                                    <p class="text-danger text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
